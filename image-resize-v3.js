...
const COLOR_MODE_KEY = "filechute-resize-color-mode";
...
function saveLikeControl(element) {
  ...
  if (element.matches(".filechute-resize-button, .filechute-output-folder-button")) return false;
  if (element.matches(".entry-save, .image-save, .filechute-save-button, [data-filechute-save-action]")) return true;
...
async function syncSaveButtons() {
  if (permissionSyncBusy || !entries) return;
  permissionSyncBusy = true;
  try {
    const keepUsingFolder = Boolean(await getRoot());
    document.body.classList.toggle("filechute-root-write-granted", keepUsingFolder);
    for (const control of entries.querySelectorAll("button, a, [role='button']")) {
      if (!saveLikeControl(control)) continue;
      control.hidden = keepUsingFolder;
      control.classList.toggle("filechute-auto-hidden-save", keepUsingFolder);
      if (keepUsingFolder) {
        control.setAttribute("aria-hidden", "true");
        control.setAttribute("tabindex", "-1");
      } else {
        control.removeAttribute("aria-hidden");
        if (control.getAttribute("tabindex") === "-1") control.removeAttribute("tabindex");
      }
    }
  } finally {
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .filechute-output-folder-button{white-space:nowrap}
    ...
    .resize-color-block{display:grid;gap:6px;margin-top:12px;color:#d6dcd8;font-size:11px}
    ...
    .resize-color-note{color:#818a84;font-size:10px;line-height:1.35}
  `;
...
function settingsInputs() {
  return {
    aspect: document.querySelector("#resize-v3-aspect"),
    noFill: document.querySelector("#resize-v3-no-fill"),
    mode: document.querySelector("#resize-v3-color-mode"),
    picker: document.querySelector("#resize-v3-default-color-picker"),
    code: document.querySelector("#resize-v3-default-color-code")
  };
}

function syncSettingsUi() {
  const { aspect, noFill, mode, picker, code } = settingsInputs();
  ...
  const colorDisabled = !maintainAspect || doNotFill;
  if (picker) {
    picker.value = defaultFillColor;
    picker.disabled = colorDisabled;
  }
  if (code) {
    code.value = defaultFillColor;
    code.disabled = colorDisabled;
  }
}
...
async function persistSettings() {
  await Promise.all([
    ...
    writeStored(COLOR_KEY, defaultFillColor),
    writeStored(COLOR_MODE_KEY, colorMode)
  ]);
}

async function loadSettings() {
  const [aspect, noFill, color, mode] = await Promise.all([
    readStored(ASPECT_KEY),
    readStored(NO_FILL_KEY),
    readStored(COLOR_KEY),
    readStored(COLOR_MODE_KEY)
  ]);
  maintainAspect = aspect !== false;
  doNotFill = noFill === true;
  defaultFillColor = normalizeHex(color) || "#000000";
  colorMode = mode === "default" ? "default" : "individual";
  syncSettingsUi();
}

function makeSettingsSection() {
  if (!settingsForm || document.querySelector("#filechute-resize-v3-settings")) return;
  const section = document.createElement("section");
  section.id = "filechute-resize-v3-settings";
  ...
  section.innerHTML = `
    ...
    <label class="setting-row checkbox-row"><span>Maintain aspect ratio</span><input id="resize-v3-aspect" type="checkbox" checked></label>
    <label class="setting-row checkbox-row"><span>Do not fill</span><input id="resize-v3-no-fill" type="checkbox"></label>
    <label class="setting-stack" for="resize-v3-color-mode">
      <span>Fill color source</span>
      <select id="resize-v3-color-mode">
        <option value="individual" selected>Choose per image</option>
        <option value="default">Always use default</option>
      </select>
      <small>Choose per image starts every resize with the default below, then lets that one image use a different color without changing your default.</small>
    </label>
    <label class="setting-stack">
      <span>Default fill color</span>
      <span class="resize-color-row">
        <input id="resize-v3-default-color-picker" class="resize-color-picker" type="color" value="#000000" aria-label="Default fill color picker">
        <input id="resize-v3-default-color-code" class="resize-color-code" type="text" value="#000000" maxlength="7" spellcheck="false" aria-label="Default fill color hex code" placeholder="#000000">
      </span>
      <small>Default: #000000. Enter a 6-digit hex code or use the color picker.</small>
    </label>
  `;
  ...
  const saveBasic = async () => {
    ...
    colorMode = mode?.value === "default" ? "default" : "individual";
    syncSettingsUi();
  ...
  mode?.addEventListener("change", () => void saveBasic());

  picker?.addEventListener("input", () => {
    defaultFillColor = normalizeHex(picker.value) || "#000000";
    if (code) code.value = defaultFillColor;
  ...
  code?.addEventListener("change", () => {
    ...
    if (!normalized) {
      code.value = defaultFillColor;
      setStatus("Default fill color must be a 6-digit hex code such as #000000.", true);
      return;
    }
    defaultFillColor = normalized;
    if (picker) picker.value = defaultFillColor;
    code.value = defaultFillColor;
    void persistSettings();
  });
}
...
async function beginSettingsDraft() {
  await loadSettings();
  settingsSnapshot = { maintainAspect, doNotFill, defaultFillColor, colorMode };
}

async function restoreSettingsDraft() {
  if (!settingsSnapshot) return;
  ({ maintainAspect, doNotFill, defaultFillColor, colorMode } = settingsSnapshot);
  await persistSettings();
  syncSettingsUi();
...
function makeDialog() {
  if (document.querySelector("#resize-v3-dialog")) return;
  const dialog = document.createElement("dialog");
  dialog.id = "resize-v3-dialog";
  dialog.innerHTML = `
    <form id="resize-v3-form" class="dialog-card">
      <div class="dialog-heading">
        <div><h2>Resize image</h2><p id="resize-v3-name">Choose an output size.</p></div>
        <button id="resize-v3-close" class="dialog-close" type="button" aria-label="Cancel resize">×</button>
      </div>
      <div class="resize-grid">
        <label>Width<input id="resize-v3-width" type="number" min="1" max="32767" step="1" required></label>
        <label>Height<input id="resize-v3-height" type="number" min="1" max="32767" step="1" required></label>
      </div>
      <label class="resize-color-block">
        <span>Fill color for this image</span>
        <span class="resize-color-row">
          <input id="resize-v3-color-picker" class="resize-color-picker" type="color" value="#000000" aria-label="Fill color for this resize">
          <input id="resize-v3-color-code" class="resize-color-code" type="text" value="#000000" maxlength="7" spellcheck="false" aria-label="Fill color hex code for this resize" placeholder="#000000">
        </span>
        <span id="resize-v3-color-note" class="resize-color-note"></span>
      </label>
      <div id="resize-v3-summary" class="resize-summary"></div>
      <div class="dialog-actions">
        <button id="resize-v3-cancel" type="button">Cancel</button>
        <button id="resize-v3-create" class="primary" type="submit">Create in resized/</button>
      </div>
    </form>
  `;
  document.body.append(dialog);

  const width = dialog.querySelector("#resize-v3-width");
  const height = dialog.querySelector("#resize-v3-height");
  const picker = dialog.querySelector("#resize-v3-color-picker");
  const code = dialog.querySelector("#resize-v3-color-code");

  width?.addEventListener("input", () => dimensionEdited("width"));
  height?.addEventListener("input", () => dimensionEdited("height"));

  picker?.addEventListener("input", () => {
    activeFillColor = normalizeHex(picker.value) || defaultFillColor;
    if (code) code.value = activeFillColor;
    updateSummary();
  });
  ...
  code?.addEventListener("change", () => {
    const normalized = normalizeHex(code.value);
    if (!normalized) {
      code.value = activeFillColor;
      setStatus("Fill color must be a 6-digit hex code such as #000000.", true);
      return;
    }
    activeFillColor = normalized;
    if (picker) picker.value = activeFillColor;
    code.value = activeFillColor;
    updateSummary();
  });

  dialog.querySelector("#resize-v3-cancel")?.addEventListener("click", () => dialog.close("cancel"));
  dialog.querySelector("#resize-v3-close")?.addEventListener("click", () => dialog.close("cancel"));
  dialog.querySelector("#resize-v3-form")?.addEventListener("submit", (event) => {
...
function dialogElements() {
  const dialog = document.querySelector("#resize-v3-dialog");
  return {
    dialog,
    width: dialog?.querySelector("#resize-v3-width"),
    height: dialog?.querySelector("#resize-v3-height"),
    name: dialog?.querySelector("#resize-v3-name"),
    picker: dialog?.querySelector("#resize-v3-color-picker"),
    code: dialog?.querySelector("#resize-v3-color-code"),
    colorNote: dialog?.querySelector("#resize-v3-color-note"),
    summary: dialog?.querySelector("#resize-v3-summary"),
    create: dialog?.querySelector("#resize-v3-create")
  };
}

function syncDialogColorUi() {
  const { picker, code, colorNote } = dialogElements();
  const fillRelevant = maintainAspect && !doNotFill;
  const individual = colorMode === "individual";
  const disabled = !fillRelevant || !individual;

  if (picker) {
    picker.value = activeFillColor;
  ...
  if (colorNote) {
    if (!fillRelevant) colorNote.textContent = "Fill is not used with the current aspect-ratio settings.";
    else if (individual) colorNote.textContent = `Starts from your default ${defaultFillColor}. Changes here apply only to this image.`;
    else colorNote.textContent = `Using the settings default ${defaultFillColor}. Switch Fill color source to Choose per image to override it here.`;
  }
}
...
function plan() {
  ...
  const out = requested();
  if (!maintainAspect) {
    return { ...out, drawX: 0, drawY: 0, drawWidth: out.width, drawHeight: out.height, fill: false, mode: "stretch" };
  }
  if (doNotFill) {
...
function updateSummary() {
  ...
  else summary.textContent = `Output: ${output.width} × ${output.height}. Aspect ratio is preserved and unused space is filled with ${activeFillColor}.`;
}
...
async function openResize(row) {
  try {
    ...
    lastEdited = "width";
    activeFillColor = defaultFillColor;
...
async function openResizedFolder(row) {
  try {
    const resolved = await resolveRow(row, { request: false });
    const currentParts = [...resolved.directoryParts];
    const alreadyResized = currentParts.at(-1)?.toLowerCase() === RESIZED_FOLDER;

    if (alreadyResized) {
      setStatus("Already in this resized/ folder.");
      return;
    }

    try {
      await resolved.directory.getDirectoryHandle(RESIZED_FOLDER);
    } catch (error) {
...
function decorateRows() {
  ...
  for (const row of entries.querySelectorAll(".entry:not(.directory)")) {
    ...
    if (!actions.querySelector(".filechute-output-folder-button")) {
      const folder = document.createElement("button");
      folder.type = "button";
      folder.className = "filechute-output-folder-button";
      folder.textContent = "📁 resized";
      folder.title = `Open the resized output folder for ${name}`;
      folder.setAttribute("aria-label", folder.title);
      folder.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        void openResizedFolder(row);
      });
